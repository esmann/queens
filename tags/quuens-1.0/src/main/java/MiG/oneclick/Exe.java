...
public class Exe implements Runnable {
	...
	public Exe(URLClassLoader urlClassLoader, String server,
			String iosessionid, String jobid, String execute_cmd,
			String[] execute_args) {
	...
	private long checkForSleepJob(String execute_str, String[] execute_args)
	...
	private void getJobMethods(Class job_class) throws NoSuchMethodException {
		...
		// Get 'boolean restoreCheckpointedFiles()' method, and check if is has
		// the right modifiers
		this.restoreCheckpointedFiles_method = job_class.getMethod(
				"restoreCheckpointedFiles", new Class[] {});
		mods = restoreCheckpointedFiles_method.getModifiers();
		if (this.restoreCheckpointedFiles_method.getReturnType() != boolean.class
				|| Modifier.isStatic(mods) || !Modifier.isPublic(mods)) {
			throw new NoSuchMethodException(
					"StringBuffer boolean restoreCheckpointedFiles()");
		}

		// Get 'void MiG_main(String[] argv)' method, and check if is has the
		// right modifiers
		this.MiG_main_method = job_class.getMethod("MiG_main",
				new Class[] { this.execute_args.getClass() });
		mods = this.MiG_main_method.getModifiers();
		if (this.MiG_main_method.getReturnType() != void.class
				|| Modifier.isStatic(mods) || !Modifier.isPublic(mods)) {
			throw new NoSuchMethodException("MiG_main(String[] argv)");
		}

		// Get the 'StringBuffer getStdout()' method, and check if is has the
		// right modifiers.
		this.getStdout_method = job_class.getMethod("getStdout", null);
		mods = this.getStdout_method.getModifiers();
		if (this.getStdout_method.getReturnType() != StringBuffer.class
				|| Modifier.isStatic(mods) || !Modifier.isPublic(mods)) {
			throw new NoSuchMethodException("StringBuffer getStdout()");
		}

		// Get the 'StringBuffer getStderr()' method, and check if is has the
		// right modifiers.
		this.getStderr_method = job_class.getMethod("getStderr", null);
		mods = this.getStderr_method.getModifiers();
		if (this.getStderr_method.getReturnType() != StringBuffer.class
				|| Modifier.isStatic(mods) || !Modifier.isPublic(mods)) {
			throw new NoSuchMethodException("StringBuffer getStderr()");
		}

		// Get the 'void setInfo(String server, String session_id)' method, and
		// check if is has the right modifiers.
		this.setInfo_method = job_class.getMethod("setInfo", new Class[] {
				this.server.getClass(), this.iosessionid.getClass(),
				this.jobid.getClass() });
		mods = this.setInfo_method.getModifiers();

		if (this.setInfo_method.getReturnType() != void.class
				|| Modifier.isStatic(mods) || !Modifier.isPublic(mods)) {
			throw new NoSuchMethodException(
					"void setInfo(String server, String session_id )");
		}
	}
	...
	private Object getCheckpointObject() {
		...
		try {
			...
			checkpoint_url_str = this.server + "/sid_redirect/"
					+ this.iosessionid + "/" + this.jobid + "."
					+ this.execute_cmd + ".checkpoint";
			checkpoint_request_url_str = checkpoint_url_str + ".latest";

			System.out.println("checkpoint_request_url_str: "
					+ checkpoint_request_url_str);

			checkpoint_request_conn = new HttpsConnection(
					checkpoint_request_url_str);
			...
			if (checkpoint_request_conn.getResponseCode() == HttpsConnection.HTTP_OK) {
				checkpoint_nr = Integer.parseInt(checkpoint_request_conn
						.readLine());
			}
			...
			if (checkpoint_nr != -1) {
				...
				checkpoint_obj_url = new URL(checkpoint_url_str + "."
						+ checkpoint_nr);
				System.out.println("checkpoint obj url: " + checkpoint_obj_url);

				// Retrieve object
				httpsUrlConn = (HttpsURLConnection) checkpoint_obj_url
						.openConnection();
				httpsUrlConn.connect();
				job_object = (Object) (new ObjectInputStream(httpsUrlConn
				...
				// Setting the executing information of this session to
				// checkpointed job
				this.setInfo_method.invoke(job_object, new Object[] {
						this.server, this.iosessionid, this.jobid });

				result = job_object;

				// Try to restore checkpointed files assosiated with this job
				filerestore_result = (Boolean) this.restoreCheckpointedFiles_method
						.invoke(job_object, new Object[] {});
				if (!filerestore_result.booleanValue()) {
					// Failed to restore checkpointed files, discarding
					// checkpoint
					result = null;
				}
			}
	...
	public void run() {
		...
		try {
			...
			if (this.execute_cmd.compareTo("sleep") == 0) {
				...
				// All Needed methods exists, if not checkpointed object, create
				// new instance of the execute_class
				if (job_object == null) {
				...
				// invoke the 'MiG_main' method of the job with the give
				// arguments.
				this.MiG_main_method.invoke(job_object,
						new Object[] { execute_args });

				job_stderr = (StringBuffer) this.getStderr_method.invoke(
						job_object, null);
				this.stderr.append(job_stderr);

				job_stdout = (StringBuffer) this.getStdout_method.invoke(
						job_object, null);
