...
public abstract class Job implements java.io.Serializable {
		...
		for (i = 0; i < this.files.size(); i++) {
			...
			if (file.getMode() == File.CLOSED && file.getFlushCount() == 0) {
				...
				for (j = 0; j < this.files.size(); j++) {
					...
					if (file != file2
							&& file.getFilename()
									.compareTo(file2.getFilename()) == 0) {
						this.files.removeElementAt(j);
						System.out.println("Removing dublee: " + j + " : "
								+ file.getFilename());
						j--;
					}
				}
		...
		try {
			...
			for (i = 0; i < this.files.size(); i++) {
				...
				if (file.getFlushCount() > 0) {
					checkpoint_name = this.jobid + "." + file.getFilename()
							+ ".checkpoint." + this.checkpoint_id;
					checkpoint_url_str = this.server
							+ "/cgi-sid/cp.py?iosessionid=" + this.iosessionid
							+ "&src=" + file.getFilename() + "&dst="
							+ checkpoint_name;

					System.out.println("checkpoint_url_str: "
							+ checkpoint_url_str);
					...
					System.out.println("New File checkpoint: '"
							+ file.getFilename() + "', rc: " + http_rc);
			...
			checkpoint_url_str = this.server + "/sid_redirect/"
					+ this.iosessionid + "/";
			checkpoint_name = this.jobid + "." + this.getClass().getName()
					+ ".checkpoint";

			url = new URL(checkpoint_url_str + checkpoint_name + "."
					+ this.checkpoint_id);
		...
		try {
			for (i = 0; i < this.files.size(); i++) {
				...
				if (file.getFlushCount() > 0) {
					...
					checkpoint_url_str = this.server
							+ "/cgi-sid/cp.py?iosessionid=" + this.iosessionid
							+ "&src=" + checkpoint_name + "&dst="
							+ file.getFilename();

					System.out.println("restore checkpoint_url_str: "
							+ checkpoint_url_str);
					url = new URL(checkpoint_url_str);
					...
					System.out.println("Restore File checkpoint copy: '"
							+ file.getFilename() + "', rc: " + http_rc);
					httpsUrlConn.disconnect();
				}
			}
	 ...
	 */
	protected MiG.oneclick.File open_file(String filename, int mode)
			throws MiG.oneclick.FileException {
		boolean status;

		MiG.oneclick.File file;

		System.out.println("Opening file: '" + filename + "' in mode: " + mode);
		file = new MiG.oneclick.File(this.server, this.iosessionid, filename,
				mode);
