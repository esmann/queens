...
Dette afsnit beskriver \nq\ i den grad det er fundet nødvendigt for parrallelisering af problemet på \mig. En kort beskrivelse af den overordnede algoritme og backtracking paradigmet beskrives i \ref{backtracking}. Repræsentation af skakbrættet som et bitmap, er beskrevet i \ref{bitmapmodellen}. Takaken's sekventielle udgave\ref{nqueen.c} og implikationer af optimeringerne beskrives kort i \ref{takalgo} og \ref{parrallel}
...
Det ses på figur \ref{fig:midterandel} at andelen af løsninger for et bræt hvor den første dronning ikke placeres i et hjørne er over 90\% og stigende for større $n$. Sammenhold med figur \ref{fig:solutionspersekund} kan vi se at antallet af løsninger er relateret til køretiden med samme faktor for forskellige $n$. Så hvis hvis der er over 90\% af løsninger i et delproblem vil det også tage over 90\% af tiden at optælle dem.
...
\subsubsection{Antallet af løsninger}
\begin{figure}[h]
\begin{center}
...
\label{fig:solrelativecount}
\end{center}
\end{figure}



I en implementation med \texttt{java.Math.BigInteger} kan vi se at der vil være et performance tab på 2\% i forhold til brugen af af den primitive type \texttt{int} \fixme{Tabel med Tid int Tid Long, Tid Biginter  og ProcentForøgelse}. 
På figur \ref{fig:solrelativecount} ses at 
...
\begin{description}
	\item[Job fejler ikke hvis der kastes en kastes en \texttt{java.lang.Error}]  Java har både \texttt{Exceptions} og \texttt{Error} der begge er af typen{java.lang.Throwable}. \oc griber kun \texttt{Exceptions} som \oc og rapportere som at jobbet er succesfuldt afsluttet. Dette kan  
	\item Ens job tvinges til at afhænge af MiG.nqueen.JoB
	\item Brug af suns egen HTTP implementation. Brug i stedet http-commons f
	...
	\itemn[CPU tid] Det er ikke mulig
