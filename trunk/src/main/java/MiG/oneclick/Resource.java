...
public class Resource implements Runnable {
	...
	public Resource(MiG.oneclick.Applet applet) throws java.lang.Exception {
		this.applet = applet;
		this.init(this.applet.getParameter("server"), this.applet
				.getParameter("sandboxkey"), this.applet
				.getParameter("resource_name"), this.applet
				.getParameter("cputime"), Boolean.valueOf(false),
				URLClassLoader.newInstance(new URL[] {}, this.applet.getClass()
						.getClassLoader()));

	}

	public void init(String server, String sandboxkey, String resource_name,
			String cputime, Boolean oneshot, URLClassLoader urlClassLoader)
		...
		this.request_joburl = this.server
				+ "/cgi-sid/requestnewjob?unique_resource_name="
				+ this.resource_name + "&cputime=" + this.cputime
				+ "&sandboxkey=" + this.sandboxkey + "&exe=jvm";
	...
	private boolean retrieveJob() {
		...
		try {
			...
			// Request job
			request_joburl = this.request_joburl + "&localjobname="
					+ this.current_jobname;
			...
			if (job_request_conn.getResponseCode() == HttpsConnection.HTTP_OK) {
				...
				if (Integer.parseInt(readline) == HttpsConnection.MIG_CGI_OK) {
					...
					// Loop until it's available, the scheduler has to create
					// the job.
					...
					while (getinputfiles_rc != HttpsConnection.HTTP_OK) {
						...
						getinputfiles_url = this.server + "/sid_redirect/"
								+ this.current_jobname + ".getinputfiles";
						System.out.println("getinputfiles_url: "
								+ getinputfiles_url);

						getinputfiles_conn = new HttpsConnection(
								getinputfiles_url);
						getinputfiles_conn.open();
						getinputfiles_rc = getinputfiles_conn.getResponseCode();

						// If jobdescription file exists read mig_iosession_id
						// and job_id from it.
						if (getinputfiles_rc == HttpsConnection.HTTP_OK) {
							...
							while (readline != null) {
								...
								if (readline.indexOf("mig_session_id: ") == 0) {
									this.mig_sessionid = readline.substring(
											readline.indexOf(": ") + 2,
											readline.length());
								} else if (readline
										.indexOf("mig_iosession_id: ") == 0) {
									this.mig_iosessionid = readline.substring(
											readline.indexOf(": ") + 2,
											readline.length());
								} else if (readline.indexOf("job_id: ") == 0) {
									this.mig_jobid = readline.substring(
											readline.indexOf(": ") + 2,
											readline.length());
								} else if (readline.indexOf("executables: ") == 0) {
									this.executables.add(readline.substring(
											readline.indexOf(": ") + 2,
											readline.length()));
								}
								readline = getinputfiles_conn.readLine();
							}
							if (this.mig_sessionid != null
									&& this.mig_iosessionid != null
									&& this.mig_jobid != null)
								result = true;
						}
		...
		if (job_rc == HttpsConnection.HTTP_OK) {
			...
			while (readline != null) {
				...
				if (readline.indexOf("execute: ") == 0) {
					execute_vector.addElement(readline.substring(readline
							.indexOf(": ") + 2, readline.length()));
				}
				readline = job_conn.readLine();
			}
		}
	...
	/*
	 * NOT used at the moment, as we can't change codebase in the default applet
	 * security model The codebase is changed by symlink changing on the server
	 *  // Return the url of the class to be executed private URL[]
	 * getExecutablesUrls() throws java.lang.Exception { for (i=0; i<this.executables.size();
	 * i++) { executable_url_str = this.server + "/sid_redirect/" +
	 * this.mig_iosessionid + "/" + (String) this.executables.elementAt(i); //
	 * Only class files is allowed as executables. // Jar files is _NOT_ allowed
	 * as they can be signed and // trick the resource to allow nasty things to
	 * occur. if (executable_url_str.indexOf(".class") ==
	 * executable_url_str.length()-6) { System.out.println("Adding: " +
	 * executable_url_str); url_vector.add(new URL(executable_url_str)); } } //
	 * Add the MiG codebase url url_vector.add(oneclick_codebase_url);
	 * 
	...
	private String[] getExecuteArgs(String[] execute_arr)
			throws java.lang.Exception {
		int i;
		String[] execute_args = null;

		if (execute_arr.length > 0) {
			execute_args = (String[]) Array.newInstance(Class
					.forName("java.lang.String"), execute_arr.length - 1);
	...
	private void printDebugInfo(String execute_cmd, String[] execute_args) {
		...
		// System.out.println("oneclick_codebase_url: " +
		// this.apple.getCodeBase());
	...
	private void retrieveExecuteImage(String execute_cmd) {
		...
		if (this.applet != null) {
			image_url_str = this.applet.getCodeBase() + execute_cmd
					+ this.execute_image_counter + ".gif";
			...
			try {
				...
				if (rc == HttpsConnection.HTTP_OK) {
					this.execute_image = this.applet.getImage(new URL(
							image_url_str));
	...
	private void execute() {
		...
		try {
			...
			i = 0;
			while (this.exit_value == MIG_JOB_EXIT_OK
					&& i < execute_vector.size()) {
				...
				exe = new Exe(this.urlClassLoader, this.server,
						this.mig_iosessionid, this.mig_jobid, execute_cmd,
						execute_args);
				...
				while (exe_thread.isAlive()) {
					...
					if (!this.checkjob.jobActive()) {
						...
						System.out
								.println("Job Execution terminated by server.");
					}
				}
	...
	private boolean sendStderr(String send_stderr_url) {
		...
		try {
			// Connect to server
			send_conn = new HttpsConnection(send_stderr_url,
					HttpsConnection.PUT);
			send_conn.open();
			...
			/*
			 * For Debug System.out.println("RC: " + stderr_rc); readline =
			 * send_conn.readLine(); while( readline != null ) {
			 * System.out.println("readline: " + readline); readline =
			 * send_conn.readLine(); }
			 */
	...
	private boolean sendStdout(String send_stdout_url) {
		...
		try {
			// Connect to server
			send_conn = new HttpsConnection(send_stdout_url,
					HttpsConnection.PUT);
			send_conn.open();
			...
			/*
			 * For Debug System.out.println("RC: " + stdout_rc); readline =
			 * send_conn.readLine(); while( readline != null ) {
			 * System.out.println("readline: " + readline); readline =
			 * send_conn.readLine(); }
			 */
	...
	private boolean sendStatus(String send_status_url) {
		...
		try {
			// Connect to server
			send_conn = new HttpsConnection(send_status_url,
					HttpsConnection.PUT);
			send_conn.open();
			...
			/*
			 * For Debug System.out.println("RC: " + status_rc); readline =
			 * send_conn.readLine(); while( readline != null ) {
			 * System.out.println("readline: " + readline); readline =
			 * send_conn.readLine(); }
			 */
	...
	private boolean sendJobFinished() {
		...
		// Retrieve information about where to deliver, stderr, stdout and
		// status
		try {
			sendfiles_url = this.server + "/sid_redirect/" + this.mig_sessionid
					+ ".sendoutputfiles";
			...
			while (readline != null) {
				...
				if (readline.indexOf("status: ") == 0) {
					send_status_url = readline.substring(
							readline.indexOf(": ") + 2, readline.length());
				} else if (readline.indexOf("stderr: ") == 0) {
					send_stderr_url = readline.substring(
							readline.indexOf(": ") + 2, readline.length());
				} else if (readline.indexOf("stdout: ") == 0) {
					send_stdout_url = readline.substring(
							readline.indexOf(": ") + 2, readline.length());
				}
				readline = sendfiles_conn.readLine();
			}
	...
	public void run() {
		...
		System.out.println("Resource: " + this.resource_name + ", sandboxkey: "
				+ this.sandboxkey);
		...
		do {
			...
			if (this.alive && this.retrieveJob()) {
				...
				if (this.alive
						&& this.exit_value != MIG_JOB_EXIT_SERVERINTERUPT) {
					this.setStatus(this.SENDING_RESULT,
							"Sending result to server.");

					// Send status to the MiG-server
					while (this.alive && this.checkjob.jobActive()
							&& !this.sendJobFinished()) {
						try {
							System.out
									.println("SendJobFinished  failed, trying again later.");
							Thread.sleep(SEND_JOB_RETRY_TIME);
						} catch (java.lang.Exception e) {
							e.printStackTrace();
						}
			...
			if (this.alive) {
				try {
					System.out
							.println("Resource loop finished, sleep for a while");
					Thread.sleep(1000);
				} catch (java.lang.Exception e) {
					e.printStackTrace();
				}
