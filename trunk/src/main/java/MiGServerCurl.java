...
public class MiGServerCurl {
	...
	public MiGServerCurl(String ServerUrl, String CaCertFileName,
			String CertFileName, String KeyFileName, String Password) {
	...
	public void MiGTouch(String filename) {
		String command = "/usr/bin/curl --fail " + this.CertFile + this.KeyFile
				+ this.CaCertFile + this.Password + " --url " + this.ServerUrl
				+ "/cgi-bin/touch.py?path=" + filename + ";with_html=false";
		System.out.println(command);
		try {
			Process pTouch = java.lang.Runtime.getRuntime().exec(command);
			BufferedReader br = new BufferedReader(new InputStreamReader(pTouch
					.getInputStream()));
			String line;
	...
	public void MiGLs(String path) {
		String command = "/usr/bin/curl --compressed --fail --silent "
				+ this.CertFile + this.KeyFile + this.CaCertFile
				+ this.Password + " --url " + this.ServerUrl
				+ "/cgi-bin/ls.py?path=" + path + ";with_html=false";
		System.out.println(command);
		try {
			Process pTouch = java.lang.Runtime.getRuntime().exec(command);
			BufferedReader br = new BufferedReader(new InputStreamReader(pTouch
	...
	public static void main(String[] args) {
		...
		MiGServerCurl mig = new MiGServerCurl(serverurl, cacertfilename,
