...
Den første placering\footnote{For hjørnebræt er dette række 2, netop fordi den første dronning skal være placeret i hjørnet} af en dronning bestemmer hvilke valg vi kan undgå, og dermed mængden af deltræ vi kan fjerne. Da der er forskellige optimeringer, \emph{bounds}, vil der være forskel på andelen af løsninger afhængig af bounds som set på figur \ref{fig:midterandel}
...
\fixme{Beskriv evt. med lidt matematik istedet.} Takakens optimeringer vil dog forandre denne fordeling. 

Det ses let at , inklusiv takakens optimeringer.  Hvert delbræt kan løses uafhængigt og antallet af løsninger for hvert bræt akkumuleres for at få det totale antal løsninger. Selve da takakens optimeringer fjerne mulige valg afhængig af den første placering
...
\fixme{Som BV siger : I bør skrive noget om at One-Click ikke er applikations specifik som BOINC}
...
\draw[snake=brace,very thick] (-0.6,-3.25) --
        node [draw=none,left=0.5em,fill=none]{$spildtid$}(-0.6,-2.5) ;
\draw[snake=brace,very thick] (0.6,-3.75) --
        node [draw=none,right=0.5em,fill=none]{$ventetid$}(0.6,-4.75) ;

\tikzstyle{every node} = [rounded corners,draw, very thick,fill=blue!20,shade]
\begin{scope}
...
\begin{description}
	\item[$JobSZ_{i}$] Beregningsstørrelsen for et job $i$.
	\item[$ResSpeed_{i}$] Hatigheden af ressource $i$.
	\item[$\#Job$] Antallet af job.
	\item[$\#Ress$] Antallet af ressourcer.  
\end{description}

Desuden defineres \emph{spildtid} som den tid en ressource benytter til at sende resultater og hente nye job. Det er denne tid der kunne spares hvis jobbets beregningstid svarede til den tid ressourcen er tilgængelig. Omvendt defineres \emph{ventetid} som den tid en ressource ikke benyttes, fordi der ikke er job klar til beregning. Se også figur \ref{fig:spildtidventetid}.
...
\begin{enumerate}
	\item $\texttt{JobSZ}_{job}$ er den samme for alle job. (Job er homogene). \label{beregningstid}
	\item $\texttt{\#Job}$ er præcis lig $\texttt{\#Ress}$.\label{antal}
	\item $\texttt{ResSpeed}_{ress}$ er den samme for alle ressourcer. (Ressourcer er homogene). 
...
\begin{itemize}
	\item $\texttt{ResSpeed}_{ress}$ er ikke kendt for nogen ressourcer (Ressourcer er heterogene).  
	\item Ressourcer er upålidelige.
	\item $\texttt{\#Ress}$ varierer.
	\item $\texttt{JobSZ}_{job}$ varierer (Job er heterogene).
\end{itemize}

Herunder antages at job er homogene, da det gør beskrivelsen væsentligt mindre kompliceret. Senere vil vi se, at denne antagelse faktisk ikke påvirker de generelle problemstillinger, men blot reducerer deres størrelsesorden.

Idet der arbejdes med heterogene ressourcer kan den minimale \emph{spildtid} ikke opnås. 
Antager man at $\texttt{\#Ress}$ er kendt vil det mindst mulige, samlede \emph{spildtid} opnås ved at have $\texttt{\#Job} = \texttt{\#Ress}$. \emph{ventetiden} vil til gengæld være stor. 

Den vil være summen af forskellene i beregningstider op til den langsommeste ressource, for alle ressourcer.
\footnote{$\texttt{ResSpeed}_{max} = max(\texttt{ResSpeed}_{1}, \ldots ,\texttt{ResSpeed}_{\#\texttt{ress}})$.}
 
\begin{displaymath}
	\sum_{i=1}^{\texttt{\#Ress}}\texttt{JobSZ} \times \frac{1}{\texttt{ResSpeed}_{max}} - 
	\texttt{JobSZ} \times \frac{1}{\texttt{ResSpeed}_{i}}
\end{displaymath}

Den samlede beregningtid vil være $\texttt{JobSZ} \times \frac{1}{\texttt{RessSpeed}_{min}} $.
\footnote{$\texttt{ResSpeed}_{min} = min(\texttt{ResSpeed}_{1}, \ldots ,\texttt{ResSpeed}_{\#\texttt{ress}})$.} 

Antager man igen at $\texttt{\#Ress}$ er kendt, vil man, med en jobmængde der er større end ressourcemængden, få en mere jævn udnyttelse af ressourcerne. Jo større $\texttt{\#Job}$ i forhold til $\texttt{\#Ress}$, desto mere jævnt bliver ressourcerne brugt, og desto mindre bliver den sammenlagte \emph{ventetid} Dette er netop fordi den jobstørrelsen falder når antallet af job stiger. 
...
\[
\texttt{\#Job} - 1 * max(JobSZ_{1}, ... , JobSZ_{\#job})
\]
.


I det ovenstående er ikke medregnet at job også er heterogene. Idet både ressourcer og job er heterogene kan de ses som et samlet problem. 
...
Medregnes det at job er heterogene, vil \emph{ventetiden} i det værste tilfælde, hvor der kun er det største job tilbage, pludselig mindst være
...
%når vi antager $\texttt{\#Ress} = \texttt{\#Job}$, og mindst

\begin{displaymath}
	\texttt{\#Ress} - 1 \times \texttt{JobSZ}_{max} \times \frac{1}{\texttt{ResSpeed}_{max}}
\end{displaymath}

%$\texttt{JobSZ}_{max} \times \frac{1}{\texttt{ResSpeed}_{min}} \times \#\texttt{Ress} - 1$\footnote{
%hvor $\texttt{JobSZ}_{max} = max( \texttt{JobSZ}_{1}, \ldots ,\texttt{JobSZ}_{\#\texttt{job}})$ 
%og $\texttt{ResSpeed}_{min} = min(\texttt{ResSpeed}_{1}, \ldots ,\texttt{ResSpeed}_{\#\texttt{ress})}$}.

Dette er en stor faktor-forøgelse i forhold til situationen, hvor job er homogene. Konklusionen på 
disse udregninger er, at jo mere homogene vores job er, jo bedre kan vi tilpasse \texttt{\#Job} så vi minimerer både \emph{vente}- og \emph{spildtid}. 

Uanset jobstørrelser, er det essentielle, at undgå ubenyttede ressourcer, uden at komme til at skrue for meget op for antallet af job, og derved øge overheadet.
...
\fixme{er benchmark output filer nødvendige at sætte ind i rapporten.. det ville fylde AAAALLT for meget og tage lang tid at lave da der er mange filer}
